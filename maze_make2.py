...
def print_header(row, col):
    ...
    print('<mujoco model="maze">\n'
          '    <worldbody>\n'
          '        <!--ベース板-->')
    print('        <body name="base_plate" pos="{} {} 0.012">'.format(pos_x, pos_y))
    print('        <geom type="plane" mass="0.1" size="{} {} 0.006" rgba="0.2 0.2 0.2 1" friction="0.0 0.0 0.0"/>'.format(size_x, size_y ) )
    print('        </body>')
...
index = 1
#Horizontal
for i in range(M+1):#南北方向
    for j in range(N):#//東西方向
        if smap[32-2*i][2+4*j]=='-':
            print_horizontal_wall(index, j * 0.18 + 0.09, i * 0.18)
            index += 1
#Vertical
for i in range(M):#南北方向
    for j in range(N+1):#//東西方向
        if smap[31-2*i][4*j]=='|':
